...
import { User, Bot, Edit3, Save, X, Volume2, VolumeX, Copy as CopyIcon, Check, AlertCircle, Loader2 } from 'lucide-react';
import IconButton from './IconButton';
import CodeEmulator from './CodeEmulator'; // Import the new component
...
const isInlineDataPart = (part: Part): part is InlineDataPart => (part as InlineDataPart).inlineData !== undefined;
...
const MessageBubble: React.FC<MessageBubbleProps> = ({
  ...
  const handleCopy = () => {
    const textToCopy = message.parts.filter(isTextPart).map(p => p.text).join('\n');
  ...
  const isModelPlaceholderLoading = !isUser && isLoading && message.parts.length === 1 && isTextPart(message.parts[0]) && message.parts[0].text.trim() === '';

  const canSaveEdit = editingState?.currentText.trim() !== '' || imageParts.length > 0;

  const bubbleBaseClasses = "px-4 py-3 rounded-lg shadow-md w-full relative group ";
  ...
  const parsedContent = useMemo(() => {
    ...
    return message.parts.reduce((acc, part) => {
      ...
      } else if (isInlineDataPart(part) && part.inlineData.mimeType.startsWith('image/')) {
  ...
  return (
    <div className={`flex ${isUser ? 'justify-end' : 'justify-start'}`}>
      <div className={`flex items-start max-w-xl lg:max-w-2xl ${isUser ? 'flex-row-reverse' : 'flex-row'}`}>
        <div className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center ${isUser ? 'ml-2 bg-indigo-500' : 'mr-2 bg-teal-500'}`}>
        ...
        <div className={bubbleClassName.trim()}>
          {isBeingEdited ? (
            <div className="space-y-2">
              ...
              {!canSaveEdit && (
                <p className="text-xs text-red-400 flex items-center"><AlertCircle size={14} className="mr-1"/>Cannot save an empty message.</p>
              )}
                  ...
                  {isUser ? (
                    message.parts.map((part, index) => {
                      if (isTextPart(part)) {
                        ...
                        const ptClass = index === 0 && (textContentForActions && textContentForActions.trim() !== '') ? 'pt-6' : '';
                        return <p key={`user-text-${index}`} className={`whitespace-pre-wrap break-words ${ptClass}`}>{paragraphs}</p>;
                      }
                      ...
                      return <p key={`user-unknown-${index}`} className="text-sm text-red-400">[Unsupported content part]</p>;
                    })
                  ) : (
                    parsedContent?.map((segment, index) => {
                      if (segment.type === 'text') {
                        ...
                        const ptClass = index === 0 && (textContentForActions && textContentForActions.trim() !== '') ? 'pt-6' : '';
                        return <p key={`model-text-${index}`} className={`whitespace-pre-wrap break-words ${ptClass}`}>{paragraphs}</p>;
                      }
                      if (segment.type === 'code') {
                       ...
                       if (segment.type === 'image') { // Render images if any were mixed in
           ...
           <p className="text-xs mt-1 opacity-70 text-right">
            ...
            {!isBeingEdited && !isModelPlaceholderLoading && (
                <div className={`absolute opacity-0 group-hover:opacity-100 transition-opacity flex items-center space-x-0.5 p-0.5 rounded-md bg-black/10 ${isUser ? 'top-1 left-1' : 'top-1 right-1'}`}>
                {isUser && (
                    <IconButton
                    icon={<Edit3 size={14} />}
                    label="Edit message"
                ...
                {!isUser && textContentForActions.trim() && (
                    <>
                    <IconButton
                        icon={speakingMessageId === message.id ? <VolumeX size={14} /> : <Volume2 size={14} />}
                        label={speakingMessageId === message.id ? "Stop speaking" : "Speak message"}
                        onClick={() => onSpeakMessage(message)}
                        className={`${speakingMessageId === message.id ? "text-red-300 hover:text-red-200" : "hover:text-green-300 text-gray-300"} p-1`}
                    />
                    <IconButton
                        icon={isCopied ? <Check size={14} /> : <CopyIcon size={14} />}
                        label={isCopied ? "Copied!" : "Copy message"}
                        onClick={handleCopy}
                        className={`${isCopied ? "text-green-300" : "hover:text-purple-300 text-gray-300"} p-1`}
                        disabled={isCopied}
                    />
                    </>
                )}
