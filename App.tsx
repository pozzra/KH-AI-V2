...
import { AlertTriangle, Info, Loader2, Edit3, Save, X, Volume2, VolumeX, Copy as CopyIcon, Check, Play, Eye, EyeOff } from 'lucide-react';
...
const isTextPart = (part: Part): part is TextPart => (part as TextPart).text !== undefined;
...
const App: React.FC = () => {
  ...
  const [inputImages, setInputImages] = useState<{ data: string; mimeType: string; name: string }[]>([]);
  ...
  const [sidebarOpen, setSidebarOpen] = useState<boolean>(() => {
    if (typeof window !== 'undefined') {
      const storedSidebarState = localStorage.getItem(LOCAL_STORAGE_KEYS.SIDEBAR_OPEN_STATE);
      if (storedSidebarState !== null) {
        try {
          return JSON.parse(storedSidebarState);
        } catch (e) {
          console.error("Failed to parse sidebarOpenState from localStorage, using default:", e);
          // Optionally, remove the corrupted item
          localStorage.removeItem(LOCAL_STORAGE_KEYS.SIDEBAR_OPEN_STATE);
          // Fall through to default screen size check
        }
      }
      return window.innerWidth >= 768; // Default to open on md screens and larger
    }
    return true; // Default for non-browser environments (less likely for this app)
  });

  const [editingState, setEditingState] = useState<{ messageId: string; currentText: string } | null>(null);
 ...
 console.log("import.meta.env is available. Checking for VITE_API_KEY...");
      ...
      if (typeof viteSpecificApiKey === 'string') {
        ...
        if (typeof genericMetaApiKey === 'string') {
          ...
          console.log("Generic API_KEY not found or not a string in import.meta.env.");
        }
      }
    } else {
    ...
    if (!apiKey && typeof process !== 'undefined' && process.env) {
      console.log("API key not found yet. Checking process.env for REACT_APP_API_KEY...");
      apiKey = process.env.REACT_APP_API_KEY;
      if (apiKey) console.log("Found REACT_APP_API_KEY in process.env:", apiKey ? "Exists" : "Empty String");
    }
    if (!apiKey && typeof process !== 'undefined' && process.env) { // General fallback
      console.log("API key not found yet. Checking process.env for generic API_KEY...");
      apiKey = process.env.API_KEY;
      if (apiKey) console.log("Found generic API_KEY in process.env:", apiKey ? "Exists" : "Empty String");
    }

    if (!apiKey) {
      setApiKeyError("API_KEY environment variable not accessible or not set. Please configure it to use the AI features.");
      ...
      try {
        ...
        setApiKeyError(error instanceof Error ? error.message : "Failed to initialize AI Service.");
      }
    }
  }, []);
  ...
  useEffect(() => {
    ...
    try {
        const storedHistoryJson = localStorage.getItem(LOCAL_STORAGE_KEYS.CHAT_HISTORY);
        if (storedHistoryJson) {
            ...
            if (Array.isArray(parsedHistory)) {
              loadedHistory = parsedHistory.sort((a, b) => b.timestamp - a.timestamp);
            }
        }
    } catch (error) {
    ...
    try {
      const activeId = localStorage.getItem(LOCAL_STORAGE_KEYS.ACTIVE_SESSION_ID);
      if (activeId) {
    ...
    if (loadedHistory.length > 0) {
      ...
      const sessionExists = loadedHistory.some(session => session.id === storedActiveSessionId);
  ...
  useEffect(() => {
    if (activeSessionId) {
      const activeSession = chatHistory.find(session => session.id === activeSessionId);
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined') {
      try {
        localStorage.setItem(LOCAL_STORAGE_KEYS.SIDEBAR_OPEN_STATE, JSON.stringify(sidebarOpen));
      } catch (error) {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      if (activeSessionId) {
        try {
          localStorage.setItem(LOCAL_STORAGE_KEYS.ACTIVE_SESSION_ID, activeSessionId);
        } catch (error) {
          console.error("Failed to save active session ID to localStorage:", error);
        }
      } else { // If activeSessionId becomes null, remove it
  ...
  const processAndSendMessages = useCallback(async (
    sessionId: string,
    ...
    try {
      const stream = await geminiService.current.generateChatResponseStream(sessionId, messagesLeadingToThisTurn);
      ...
      setCurrentMessages([...messagesLeadingToThisTurn, placeholderUiMessage]);
      updateChatSession(sessionId, [...messagesLeadingToThisTurn, placeholderUiMessage], titleToUpdate);

      for await (const chunk of stream) {
        modelResponseText += chunk.text;
        const streamingUiMessage: ChatMessage = { ...modelMessageBase, parts: [{ text: modelResponseText }], timestamp: Date.now() };
        
        setCurrentMessages([...messagesLeadingToThisTurn, streamingUiMessage]);
        updateChatSession(sessionId, [...messagesLeadingToThisTurn, streamingUiMessage], titleToUpdate);
      }
      
      const finalUiMessage: ChatMessage = { ...modelMessageBase, parts: [{ text: modelResponseText }], timestamp: Date.now() };
      setCurrentMessages([...messagesLeadingToThisTurn, finalUiMessage]);
      updateChatSession(sessionId, [...messagesLeadingToThisTurn, finalUiMessage], titleToUpdate);
  ...
  const handleSendMessage = useCallback(async () => {
    ...
    if (!currentActiveSessionId) {
        alert("Failed to establish an active chat session. Please try refreshing or creating a new chat.");
        return;
    }
    ...
    const currentSessionForTitle = chatHistory.find(s => s.id === currentActiveSessionId);
    sessionTitleToUpdate = currentSessionForTitle?.title; 

    if (currentSessionForTitle && currentSessionForTitle.messages.length === 0 && inputText.trim()) {
        sessionTitleToUpdate = inputText.trim().substring(0, 30) + (inputText.trim().length > 30 ? "..." : "");
    }
    
    setCurrentMessages(messagesIncludingUser); 
    updateChatSession(currentActiveSessionId, messagesIncludingUser, sessionTitleToUpdate);

    setInputText('');
    setInputImages([]);
    
    await processAndSendMessages(currentActiveSessionId, messagesIncludingUser, sessionTitleToUpdate);

  }, [inputText, inputImages, activeSessionId, updateChatSession, chatHistory, currentMessages, handleNewChat, processAndSendMessages]);
  ...
  const handleEditInputChange = useCallback((newText: string) => {
    setEditingState(prev => prev ? { ...prev, currentText: newText } : null);
  }, []);
  ...
  const handleSaveEdit = useCallback(async () => {
    ...
    const messageIndex = activeChatSession.messages.findIndex(msg => msg.id === messageId);
    if (messageIndex === -1) return;

    const originalMessage = activeChatSession.messages[messageIndex];
    
    const existingImageParts = originalMessage.parts.filter(part => !isTextPart(part));
    ...
    const updatedMessage: ChatMessage = {
      ...originalMessage,
      parts: currentText.trim() ? [updatedTextPart, ...existingImageParts] : existingImageParts,
      timestamp: Date.now(),
    };

    const messagesForResubmission = activeChatSession.messages.slice(0, messageIndex).concat(updatedMessage);
    
    let sessionTitleToUpdate = activeChatSession.title;
    if (messageIndex === 0 && currentText.trim()) { 
        sessionTitleToUpdate = currentText.trim().substring(0, 30) + (currentText.trim().length > 30 ? "..." : "");
    }

    setCurrentMessages(messagesForResubmission); 
    updateChatSession(activeSessionId, messagesForResubmission, sessionTitleToUpdate);
    setEditingState(null);

    await processAndSendMessages(activeSessionId, messagesForResubmission, sessionTitleToUpdate);

  }, [editingState, activeSessionId, chatHistory, updateChatSession, processAndSendMessages]);
        ...
        .then(newImagesData => {
          setInputImages(prevInputImages => {
            ...
            if (combinedImages.length > 5) {
                ...
                console.warn(`Maximum 5 images allowed. You had ${prevInputImages.length}, selected ${numAttempted}. Added ${numCanAdd} new image(s).`);
            }
  ...
  const initializeSpeechRecognition = useCallback(() => {
    ...
    if (!SpeechRecognitionAPI) {
      ...
      alert("Speech recognition is not available or not supported by your browser.");
      return null;
    }
    ...
    recognition.onresult = (event: SpeechRecognitionEvent) => {
        ...
        for (let i = event.resultIndex; i < event.results.length; ++i) {
            ...
            if (event.results[i].isFinal) {
                const space = (combinedFinalizedFromEvent.length > 0 &&
        ...
        if (combinedFinalizedFromEvent) {
            const spaceBeforeAppending = (speechBaselineTextRef.current.length > 0 &&
                                        !speechBaselineTextRef.current.endsWith(' ') &&
                                        combinedFinalizedFromEvent.length > 0 &&
                                        !combinedFinalizedFromEvent.startsWith(' ')) ? ' ' : '';
            speechBaselineTextRef.current += spaceBeforeAppending + combinedFinalizedFromEvent;
        ...
        if (latestInterimFromEvent) {
            const spaceForInterim = (displayText.length > 0 &&
  ...
  const toggleRecording = () => {
    ...
    if (isRecording) {
      ...
      try {
        ...
        alert("Could not start recording. Please check microphone permissions and ensure it's not already in use.");
      }
    }
  };
  ...
  const handleSpeakMessage = useCallback((message: ChatMessage) => {
    ...
    const textToSpeak = message.parts.filter(isTextPart).map(p => p.text).join(' ');
    if (!textToSpeak.trim()) return;

    const utterance = new SpeechSynthesisUtterance(textToSpeak);
    utterance.lang = 'km-KH'; 
    
    const khmerVoices = availableVoices.filter(voice => voice.lang === 'km-KH' || voice.lang.startsWith('km-'));
    if (khmerVoices.length > 0) {
      utterance.voice = khmerVoices[0]; 
      console.log(`Using Khmer voice: ${khmerVoices[0].name} (${khmerVoices[0].lang})`);
    } else {
      console.warn("No specific Khmer (km-KH) voice found. Using browser default for km-KH or fallback.");
    }
  ...
  const deleteSession = (sessionId: string) => {
    setChatHistory(prev => {
      const filteredHistory = prev.filter(session => session.id !== sessionId);
    ...
    return (
      <div className="flex items-center justify-center h-screen bg-gray-900 text-red-400 p-8">
        <div className="bg-gray-800 p-6 rounded-lg shadow-xl text-center">
          ...
          <p>{apiKeyError}</p>
          <p className="mt-4 text-sm text-gray-400">Please ensure the API_KEY is correctly set up in your execution environment and refresh the application.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-gray-800">
      ...
      />
      <div className={`flex-1 flex flex-col transition-all duration-300 ease-in-out ${sidebarOpen && window.innerWidth >= 768 ? 'ml-64' : 'ml-0'}`}>
        {activeSessionId && chatHistory.find(s => s.id === activeSessionId) ? ( 
          <ChatInterface
